package com.giri.aiart.shared.util;

import lombok.experimental.UtilityClass;

/// A set of Log Icons to give logs some artistic touch.
///
/// @author Giri Pottepalem
@UtilityClass
public class LogIcons {
    public final String STARTUP = "🚀";
    public final String AI = "✨";
    public final String CHAT ="💬";
    public final String PIN ="📌";
    public final String PENCIL = "✏️";
    public final String DOCUMENT = "📄";
    public final String SEED = "🌱";
    public final String FILE = "📂";
    public final String DATA = "📥";
    public final String ARTIST = "🎨";
    public final String ANALYSIS = "📊";
    public final String SEARCH = "🔍";
    public final String TEXT = "📝";
    public final String TIMER = "🕓";
    public final String CONTROLLER = "🌐";   // Controller / Web Layer
    public final String SERVICE = "🛠️";      // Service / Business Logic Layer
    public final String PERSISTENCE = "🗄️";  // Persistence / Repository Layer
    public final String UTILITY = "⚡";       // Utility / Helper / Misc Layer
    public final String CONFIG = "📝";       // Startup / Configuration / System Info
    public final String DEBUG = "🔍";        // debug / verbose / trace
    public final String SUCCESS = "✅";      // success / completion
    public final String WARNING = "⚠️";      // warning / completion
    public final String ERROR = "❌";        // error / failure
    public final String BAR_CHART = "📊";
    public final String LINE_CHART = "📈";
    public final String TEST = "🧪";
}
